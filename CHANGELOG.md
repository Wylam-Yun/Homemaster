...
- 问题：V1.7 的 THOR trial 身份、reset 扫描、模型当前视图、外部动作终态、Provider 重试和 Runner 计数由多个边界分别推断，可能把 Harness/运行时失败混入 Agent 评分，也无法证明模型动作只使用该次成功请求实际看到的图片。
  ...
  验证：新增 committed-manifest 回归后，计划聚焦命令 `193 passed`，全仓 `395 passed, 1 skipped`；changed Python files Ruff、compileall、cleanup/interface/V1.8 guards、JSON/Markdown/secret/whitespace 检查通过。全改动集 format check 保留既有 baseline，不做批量格式化。Gate A 保留 19/20；Gate B `run-001` 暴露并修复 keyword-only 调用，`run-002` 真实进入 THOR 后以 `scan_pose_mismatch -> scan_time_scale_restore_rejected`、5 setup/0 model requests 终止。直接审查新增 runtime-scene 校验及两个回归后，`run-003` 通过正确 FloorPlan219 身份门并复现同一终态；独立 verifier 为 `overall_status=incomplete`、exit 2。Mimo profile 经真实 `LLMClient` 请求返回 `OK`；固定十 Episode run 完整退出，但 10/10 均在同一 reset recovery terminal，合计 50 setup、0 model/Provider requests，0% evaluation/Harness coverage，`formal_score_available=false`，不记为 PASS。
