...
- 新建 `memory_index.py`
- 新建 `memory_rag.py`
- 修改 `contracts.py`：保留新链路契约 `MemoryRetrievalQuery`、`MemoryRetrievalResult`、`GroundedMemoryTarget`、`PlanningContext`；删除新 `homemaster` 链路里的旧 `ObjectMemorySearchPlan`、`ObjectMemoryEvidence`、`Candidate`、`CandidatePool`、`CandidateSelection`、`selected_candidate_id`、`switch_candidate`。旧 `src/task_brain` 的 candidate 逻辑只作为 V1 对照基线保留，不接入 V1.2 新链路。
...
- Stage 05 输出中不得出现 `selected_candidate_id`、`candidate_id`、`CandidatePool`、`CandidateSelection` 或 `switch_candidate`；旧 candidate 逻辑只允许存在于旧 `src/task_brain` 基线，不允许进入 V1.2 新链路。
...
说明：原计划中的独立阶段 6-8 已合并进阶段 5。阶段 6 直接进入任务总结和记忆写回。
...
- case：`scenario_matrix_stale_recover`
  ...
  - 预期：LLM 驱动当前 grounded target 失败后的 `retrieve_again`，带 negative evidence 重新生成 PlanningContext，并成功或给出合理失败结论。
...
- 精炼结论报告保存到 `record/YYYY-MM-DD-stage-07-scenario-matrix.md`
