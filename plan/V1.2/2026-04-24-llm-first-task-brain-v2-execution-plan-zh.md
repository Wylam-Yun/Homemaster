...
用户输入
  ...
  -> RAG hit grounding / planning context 组装
  -> 高层模块编排
  -> 决策 / 执行 / 验证 / 恢复闭环
  -> 具身操作接口
  -> 任务总结
...
- 把 top executable RAG hit ground 成可执行导航目标
...
- 新增 embedding provider 配置，优先放在 `config/nvidia_api_config.json` 或独立的 `config/embedding_config.json`；后续接入用户提供的 BGE-M3 API，但日志和 debug 资产不能保存 API key。
...
职责：程序把 top executable RAG hit ground 成可导航、可验证的执行目标。
...
- Grounded target 由程序从 `MemoryRetrievalResult.hits` 的 top executable hit 生成。
- `memory_id`、anchor 和 viewpoint 必须能在当前 scenario / world 数据里找到。
- 如果 top hit 缺失 viewpoint、已被 negative evidence 排除或不可导航，程序选择下一个 executable hit；如果没有可执行 hit，进入恢复/追问/失败路径。
...
- Stage 04 不要求 Mimo 再做单独目标选择。
- `selected_target` 默认来自 RAG 排序后的 top executable hit。
- `memory_evidence` 和 `rejected_hits` 需要保留足够 score breakdown，便于 Stage 05 prompt 和 debug 报告解释为什么去这个位置。
...
- 程序默认选择 top executable RAG hit 作为 grounded target；模型不再单独做目标选择。
...
### 6.3 RAG hit grounding 与 planning context 组装
...
- 程序从 `MemoryRetrievalResult.hits` 中选择第一个 executable hit。
- 程序校验该 hit 的 `memory_id`、anchor、room、viewpoint 是否存在且可导航。
- 程序把 selected hit ground 成 `GroundedMemoryTarget`。
- 程序组装 `PlanningContext`，包含 `TaskCard`、retrieval query、memory evidence、selected target、rejected hits、runtime state summary 和 world summary。
- 如果没有 executable hit，进入恢复/重检索/追问/失败路径；不让模型临时编造地点。

测试：

- top executable hit 能在 scenario 中找到真实 viewpoint。
- 缺失 viewpoint 的 memory 不能成为 `GroundedMemoryTarget`。
- 被 negative evidence 排除的 hit 不能成为 `GroundedMemoryTarget`。
- `PlanningContext` 中包含足够的 memory evidence 和 world summary，Stage 05 不需要回头读原始 memory JSON。
...
- 程序校验计划中引用的 target/viewpoint/module 都来自 `PlanningContext`。
...
- 新建 `memory_index.py`
- 新建 `memory_rag.py`
- 修改 `contracts.py`：移除 Stage 01 预留的 `ObjectMemorySearchPlan` 壳，新增 `MemoryRetrievalQuery` 和 `MemoryRetrievalResult`，让代码契约与 Stage 03 RAG 路线一致。
...
- contract 迁移测试确认 `MemoryRetrievalQuery` / `MemoryRetrievalResult` 可序列化、可校验，且 `src/homemaster/contracts.py` 不再导出 `ObjectMemorySearchPlan`。
...
- case：`memory_rag_contract_migration`
  ...
  - 预期：`MemoryRetrievalQuery`、`MemoryRetrievalResult` 均可 `model_dump_json()` / `model_validate_json()`；静态检查确认不再使用 `ObjectMemorySearchPlan`。
...
- `contracts.py` 已完成契约迁移：Stage 03 代码和测试使用 `MemoryRetrievalQuery` / `MemoryRetrievalResult`，不再使用 `ObjectMemorySearchPlan`。
...
### 阶段 4：RAG hit grounding 与 planning context 组装

实现：

- 新建 `grounding.py`
- 新建 `planning_context.py`
- 从 `MemoryRetrievalResult.hits` 中选择 top executable hit。
- 将 selected hit 转成 `GroundedMemoryTarget`。
- 组装 Stage 05 使用的 `PlanningContext`。
- Stage 04 不要求 Mimo 进行单独目标选择，也不要求 reranker；排序来源仍是 Stage 03 的 BM25+BGE-M3 融合结果。
...
- 缺 anchor/viewpoint 的 hit 不能成为 selected target。
...
- case：`ground_medicine_target`
  ...
  - 预期：程序选择 top executable hit，生成 `GroundedMemoryTarget(memory_id=..., viewpoint_id=...)`，不调用 Mimo candidate selection。
...
- case：`skip_invalid_viewpoint_hit`
  ...
  - 预期：top hit 进入 rejected hits，程序选择第二个 executable hit。
- case：`planning_context_for_orchestration`
  - 输入：`TaskCard` + `MemoryRetrievalResult` + selected target + world summary
  - 预期：`PlanningContext` 包含 task、query、selected target、memory evidence、rejected hits、runtime summary、world summary；Stage 05 prompt 不需要读取原始 memory JSON。
- debug 资产：`tests/homemaster/llm_cases/stage_04/<case_name>/`

验收标准：

- `GroundedMemoryTarget` 由程序从 Stage 03 RAG hits 真实生成。
- selected target 只接受带 `memory_id`、anchor 和 viewpoint metadata 的 executable RAG hit。
- Stage 04 不要求 Mimo 输出单独选择结果；直接选择 top executable hit 是允许且推荐的默认策略。
- `PlanningContext` 是 Stage 05 的唯一上下文入口，包含编排所需最小信息。
- 如果所有 hits 都不可执行，Stage 04 必须输出明确失败/重检索原因，不能伪造地点。
...
- OrchestrationPlan 引用的 navigation target / viewpoint 必须来自 `PlanningContext.selected_target`。
...
- Stage 04 将 top executable RAG hit ground 成可执行目标，并组装 PlanningContext。
