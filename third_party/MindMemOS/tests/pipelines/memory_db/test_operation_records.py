...
@pytest.mark.asyncio
async def test_operation_recorder_uses_injected_qdrant_for_search_records() -> None:
    class FakeQdrant:
        def __init__(self) -> None:
            self.records = []

        async def upsert_search_record(self, point) -> None:
            self.records.append(point)

    qdrant = FakeQdrant()
    recorder = MemoryOperationRecorder(
        add_record_store=FakeAddRecordStore(),
        clients=type("Clients", (), {"qdrant": qdrant})(),
    )
    ctx = make_context()
    timestamp = datetime.now(UTC)

    await recorder.record_search(
        SearchPipelineInput(query="remember?"),
        SearchPipelineResult(status="ok", memories=[]),
        ctx=ctx,
        request_submitted_at=timestamp,
        task_completed_at=timestamp,
    )

    assert len(qdrant.records) == 1
