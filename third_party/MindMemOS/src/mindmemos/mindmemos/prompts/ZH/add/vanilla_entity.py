...
- 先按任务或事件组织信息，再生成记忆；不要按消息数量或工具调用次数生成记忆。
- 同一目标、同一对象上的连续操作，如果构成因果链、修复链或验证链，必须合并为一条候选记忆，例如“发现问题 → 修改 → 再次验证”“创建文件 → 读取检查 → 修复 → 最终确认”“命令失败 → 分析错误 → 更换方案 → 执行成功”。上述步骤即使每一步单独能够读懂，也仍属于同一事件，不得按单次工具调用拆分。
- 只有信息具有不同目标或不同对象，且脱离其他步骤后仍具有独立复用价值时，才分别生成多条记忆。不要机械拆分，也不要重复复述。

【抽取标准】
优先抽取未来可能复用的信息：稳定身份/偏好/长期约束；项目、工具、文件、配置、版本、需求、决定、任务状态；具有独立复用或排障价值的工具操作链、关键参数、异常原因和验证结论；已明确表达或验证的经验、失败原因、方法、流程与恢复策略；会影响后续决策或协作的明确计划、关切、反应和已采纳建议。
单次普通工具调用通常只是任务证据，不应仅因发生了一次调用就生成一条记忆。
...
- tool_trace：具有独立复现或排障价值的完整工具操作链。同一任务中的连续调用、参数、错误、修复和验证结果应合并为一条；普通读取、写入、确认成功等中间步骤仅作为 source_refs 证据，不得分别生成记忆。只有某次工具调用自身具有独立价值，例如出现特殊错误、发现环境限制、确定关键参数或形成可复用命令时，才允许单独生成 tool_trace。
...
【去重、关联与 action_hint】
- 生成最终结果前先在本批 extractable 内聚合：语义等价或存在包含关系的候选合并；属于同一任务、同一对象、同一因果链或验证链的候选合并，并汇总直接证据的 source_refs。
- 如果一条 experience 或 skill_candidate 已完整包含具体工具步骤，不再重复保存缺乏独立复用价值的 tool_trace。mem_type 不同不是拆分理由；应先判断是否属于同一事件，再为合并后的候选选择最具体的类型。
- 对于一次目标单一、对象单一的短任务，通常只产生 1～3 条记忆：一条任务结果或事件记忆，以及零到两条真正可独立复用的经验、事实或工具轨迹。不要为了覆盖每条证据而增加记忆数量。
