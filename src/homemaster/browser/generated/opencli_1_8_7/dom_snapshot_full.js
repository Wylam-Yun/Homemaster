...
function compoundInfoOf(el) {
  ...
  // ── Config ─────────────────────────────────────────────────────────
  const VIEWPORT_EXPAND = 800;
  const MAX_DEPTH = 50;
  const INTERACTIVE_ONLY = false;
  const MAX_TEXT_LEN = 120;
  const INCLUDE_SCROLL_INFO = true;
  const BBOX_DEDUP = true;
  const INCLUDE_SHADOW_DOM = true;
  const INCLUDE_IFRAMES = true;
  const MAX_IFRAMES = 5;
  const PAINT_ORDER_CHECK = true;
  const ANNOTATE_REFS = true;
  const REPORT_HIDDEN = true;
  const FILTER_ADS = true;
  const MARKDOWN_TABLES = true;
  const PREV_HASHES = null;
