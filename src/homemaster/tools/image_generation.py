...
_CODEX_CLAIM = "https://api.openai.com/auth"
...
class ImageGenerationToolInput(BaseModel):
    ...
    image_paths: list[str] = Field(
        ...
        description=(
            "Local image paths to edit or use as references. OpenAI provider uses image "
            "edit mode. Codex hosted generation currently treats these as visual context."
        ),
    )
...
class ImageGenerationTool(BaseTool):
    ...
    async def _generate_with_openai(
        ...
        if not api_key:
            raise RuntimeError(
                ...
                "or HOMEMASTER_IMAGE_GENERATION_API_KEY, or choose provider='codex'."
            )
    ...
    async def _generate_with_codex(
        ...
        if not auth_token:
            raise RuntimeError(
                ...
                "or use provider='openai' with HOMEMASTER_IMAGE_GENERATION_API_KEY."
            )
        ...
        async with httpx.AsyncClient(timeout=180.0, follow_redirects=True) as client:
            async with client.stream("POST", url, headers=headers, json=body) as response:
                if response.status_code >= 400:
                    ...
                    raise RuntimeError(
                        detail or f"Codex request failed: {response.status_code}"
                    )
    ...
    async def _edit_images(
        ...
        image_handles = [
            Path(path).expanduser().resolve().open("rb")
    ...
    def _resolve_output_paths(arguments: ImageGenerationToolInput, cwd: Path) -> list[Path]:
        ...
        if arguments.n == 1:
            return [base]
        return [
            base.with_name(f"{base.stem}-{idx}{base.suffix}")
            for idx in range(1, arguments.n + 1)
        ]
...
def _resolve_codex_url(base_url: str | None) -> str:
    ...
    if value and "chatgpt.com/backend-api" not in value:
        value = ""
    root = (value or _CODEX_BASE_URL).rstrip("/")
    if root.endswith("/codex/responses"):
        return root
    if root.endswith("/codex"):
        return f"{root}/responses"
    return f"{root}/codex/responses"
...
def _build_codex_headers(token: str) -> dict[str, str]:
    ...
    try:
        payload = json.loads(
            base64.urlsafe_b64decode(parts[1] + "=" * (-len(parts[1]) % 4)).decode()
        )
        account_id = payload[_CODEX_CLAIM]["chatgpt_account_id"]
    except (KeyError, ValueError, TypeError, json.JSONDecodeError) as exc:
        raise ValueError("Codex access token is missing account metadata") from exc
    if not isinstance(account_id, str) or not account_id:
        raise ValueError("Codex access token is missing chatgpt_account_id")
    return {
        "Authorization": f"Bearer {token}",
        "chatgpt-account-id": account_id,
        "originator": "homemaster",
        "User-Agent": f"homemaster ({platform.system().lower()} {platform.machine()})",
