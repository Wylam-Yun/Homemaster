"""object_memory-only RAG retrieval."""
...
class MemoryRagBoundaryError(MemoryRagError):
    """Raised when an LLM query violates boundaries."""
    ...
    *,
    ...
    if not passed:
        raise MemoryRagError(
            ...
            message=f"case {case_name!r} did not match expected fields.",
        )
    ...
    *,
    ...
    if case_name not in cases:
        raise MemoryRagError(
            ...
            message=f"unknown case: {case_name}",
        )
...
def _write_retrieval_markdown(
    ...
    *,
    ...
    text = f"""# Memory RAG - {actual["case_name"]}
