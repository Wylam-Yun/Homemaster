...
`Qdrant/bm25` 在启动时只从本地 fastembed cache 加载，并核对锁定 commit、文件集合、SHA-256 和中文编码。
冷缓存或损坏缓存会让五个 mem0 工具返回 `memory_backend_unavailable`，不会启动时联网下载或退化为
semantic-only。文件 `memory` 工具仍可用。

## 六个工具

### `memory`

读取或原子修改 USER/MEMORY。`target=user|memory`；单操作使用 `action=read|add|update|delete`，
`update/delete` 用唯一 `match`；多操作使用 `operations` batch。读要求 `tool.read`，任何 mutation 另要求
`tool.mutate`。
...
`search_memories` 合并 metadata exact、SiliconFlow semantic 和本地 Qdrant BM25，并返回来源标签。默认 limit
