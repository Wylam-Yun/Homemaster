...
真实 Run 32 中模型根据只读 CSS/DOM 结果构造点击目标时发送了 `role: ""`。`Target.from_value()` 将空字符串
当成已指定的 semantic role，resolver 因而把一个不存在的角色当作硬约束，返回 `target_not_found`；这不是
页面缺少目标，也不是 stable ref 失效。

### 修法与教训

`Target.from_value()` 现在把空的 `role`、`name`、`label`、`text` 和 `testid` 归一化为 `None`，只有非空
semantic field 才参与匹配。只读 inspect 结果不得把缺失字段序列化成约束；协议边界应统一区分“字段未提供”和
“字段提供但为空”，并用一次 direct semantic action + 外部 DOM 终态回读验证。

### Ref

- `src/homemaster/browser/contracts.py`
- `tests/homemaster/browser/test_targets.py`
- `/tmp/homemaster/v31-run32-20260826-083000/`

## 2026-08-26 - Semantic collection timeout during AntD picker re-render

### 症状与根因

真实 Run 32 中日期节点已经存在且 stable ref 可以点击，但 direct semantic action 和 `browser_inspect`
在弹层重渲染时扫描整页约 300 个交互节点，超过 10 秒 action/inspect timeout，随后 Runtime fence。
同一页面的 role-specific query 只需扫描日期或 option 节点，说明问题是采集范围与渲染竞态，而不是目标
不存在。

### 修法与教训

`collect_elements` 根据请求 role 使用窄 selector（cell/gridcell/option 等），semantic action 将 target
的 role/name/label/text/testid/frame 过滤传入采集，减少整页扫描窗口。新增真实 Playwright popup 回归并
独立探针确认 24 个 hour option 节点均可见。语义查询必须在采集入口就缩小候选集；仅在 resolver 层
增加兼容规则不能修复整页采集超时。
