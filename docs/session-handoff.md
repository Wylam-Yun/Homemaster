...
- V3.1 implements 27 safe one-tool-per-file browser tools and separately gated `browser_eval`. Provider
  ...
  profile, runtime observation barrier and context projection use `browser_screenshot`, semantic targets,
  and stable refs; browser `observe` and required pre-write `snapshot_id/element_id` are absent.
